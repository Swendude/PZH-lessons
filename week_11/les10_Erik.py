...
for rij in range(hoogte):
    for cel in range(breedte):        
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                if not(i == 0 and j == 0): 
                    check_cel = (doelcel[0] + i, doelcel[1] + j)
                    if check_cel[0] == hoogte or check_cel[1] == breedte:
                        check_cel = (check_cel[0] - hoogte, check_cel[1])
                    # if check_cel[0] == -1 or check_cel[1] == -1:
                    #     break
                    elif wereld[check_cel[0]][check_cel[1]] == 1:
                        aantal_buren += 1
